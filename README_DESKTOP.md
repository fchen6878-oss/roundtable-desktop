...
## 打包为 Windows 安装包
```bash
npm run dist       # 产出 dist/圆桌会议-Setup-1.0.0.exe（NSIS 安装包）
```
...
**构建已验证通过**，产物 `dist/圆桌会议-Setup-1.0.0.exe`（约 75 MB）。
...
- 发布后会在仓库 Releases 生成 `v1.0.0` 标签，含 `圆桌会议-Setup-1.0.0.exe`、`.blockmap`、`latest.yml`。
...
## 应用图标
已生成占位图标 `build/icon.ico`（256×256，蓝底 + 三色圆点，寓意多方讨论）。
如需正式图标，替换该文件（建议含 16/32/48/128/256 多分辨率）后重新 `npm run dist` 即可。
